...
Cross CPU finding: the same full range U8S8 model produced maximum absolute error0.01849 locally but0.21932 on a CI runner. Both float32 variants stayed within5.97e-7. This matches the ONNX Runtime documented VPMADDUBSW saturation risk on non-VNNI x86. Set reduce_range=True, quantizing weights to7bits in int8 storage. A pair of maximum unsigned8 and signed7 values cannot exceed signed16 accumulation bounds. The local error rises to0.02837 but remains within the original0.04 gate; requalify across CI CPUs instead of relaxing tolerances.
