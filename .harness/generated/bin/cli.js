...
// Generated by metaharness — the `onnx-agent-agent` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx onnx-agent-agent`
...
// "bin" + "files" fields in package.json), so `npx onnx-agent-agent` works the moment
...
const HARNESS_NAME = 'onnx-agent-agent';

/** `onnx-agent-agent init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `onnx-agent-agent doctor` — verify the install end-to-end (kernel + host resolve). */
