...
def qualify(iterations=100,provider='CPUExecutionProvider'):
    ...
    data,w,b=fixture(); fp=session(data,provider)
    ...
    results={}
    for name,blob,s in [('float32',data,fp),('int8',quant,qs)]:
        ...
        for i in range(iterations):
            start=time.perf_counter_ns(); s.run(None,{'input':inputs[i%16]}); times.append((time.perf_counter_ns()-start)/1e6)
        results[name]={'sha256':hashlib.sha256(blob).hexdigest(),'bytes':len(blob),'maxAbsoluteError':max(errors),'p50Ms':float(np.percentile(times,50)),'p95Ms':float(np.percentile(times,95))}
    passed=results['float32']['maxAbsoluteError']<=1e-5 and results['int8']['maxAbsoluteError']<=.04
    return {'schemaVersion':1,'fixture':'seeded-linear-64x32','provider':provider,'availableProviders':ort.get_available_providers(),'iterations':iterations,'holdoutBatches':16,'results':results,'parityPass':passed,'automaticPromotion':False,'productionQualified':False,'versions':{'onnx':onnx.__version__,'onnxruntime':ort.__version__,'numpy':np.__version__}}
