...
class PipelineTests(unittest.TestCase):
 def test_real_cpu_parity(self):
  r=qualify(20);self.assertTrue(r['parityPass']);self.assertLess(r['results']['int8']['bytes'],r['results']['float32']['bytes'])
 ...
 def test_cli(self):
  r=subprocess.run([sys.executable,'-m','onnx_agent','qualify','--iterations','2'],capture_output=True,text=True);self.assertEqual(r.returncode,0,r.stderr);self.assertTrue(json.loads(r.stdout)['parityPass'])
